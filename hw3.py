...
def lin_comb_mat_vec_mult(M, v):
    ...
    for key, val in colVecs.items():
        colVecs[key] = v[key] * colVecs[key]
    return sum(colVecs.values())
...
def lin_comb_vec_mat_mult(v, M):
    ...
    for key, val in rowVecs.items():
        rowVecs[key] = v[key] * rowVecs[key]
    return sum(rowVecs.values())
...
def dot_product_mat_vec_mult(M, v):
    assert(M.D[1] == v.D)
    rowVecs = matutil.mat2rowdict(M)
    return Vec(M.D[0], {key:v * row for key,row in rowVecs.items()})
...
def dot_product_vec_mat_mult(v, M):
    assert(v.D == M.D[0])
    colVecs = matutil.mat2coldict(M)
    return Vec(M.D[1], {key:v * col for key,col in colVecs.items()})
...
def Mv_mat_mat_mult(A, B):
    ...
    return matutil.coldict2mat({key:A * val for key, val in bCols.items()})
...
def vM_mat_mat_mult(A, B):
    ...
    return matutil.rowdict2mat({key:val * B for key, val in aRows.items()})
...
def dot_prod_mat_mat_mult(A, B):
    assert A.D[1] == B.D[0]
    return Mat((A.D[0], B.D[1]), {(r,c):row * col for r,row in matutil.mat2rowdict(A).items() for c,col in matutil.mat2coldict(B).items()})
