...
def scale_vecs(vecdict):
    ...
    '''
    retList = []
    for scale, vec in vecdict.items():
        retList.append(Vec(vec.D, {key:val/scale for key,val in vec.f.items()}))
    return retList
...
def GF2_span(D, L):
    ...
    gf2Set = [0, one]
    if n > 3:
        alphaList = [[x,y] for x in gf2Set for y in gf2Set]
        for i in range(n-2):
            alphaList = [x + [y] for x in alphaList for y in gf2Set]
    elif n == 2:
        alphaList = [[x,y] for x in gf2Set for y in gf2Set]
    elif n == 1:
        return [Vec(D, {}), L[0]]
    
    for alphas in alphaList:
        retList.append(sum([a * vec for (a, vec) in zip(alphas, L)]))
